...
getAllGOTerms <- function(go_df){
    ...
    ##################################################################################
    getAllMappings <- function(go_ids, ontologies){
      go2Ancester <- NULL
      print(sprintf("Getting all ancestor GO terms for %i observed terms. This may take a while!", length(go_ids)))
      pb <- txtProgressBar(min = 0, max = length(go_ids), style=3)
      for (i in 1:length(go_ids)){
        query <- go_ids[i]
        
        go_ancestors <- tryCatch(get(query, determineAncesterFunction(query, ontologies)), error = function(e) e)
        
        if(class(go_ancestors)=="character"){
          go2Ancester[[go_ids[i]]] <- go_ancestors
        }
        else{
          print(go_ancestors)
          print(sprintf("Could not get ancestors for GO.ID=%s", query))
          go2Ancester[[go_ids[i]]] <- c(NA)
